...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const checkAuthWithToken = async (tokenToCheck: string): Promise<boolean> => {
    try {
      const response = await fetch("http://14.187.198.210:12122/api/users/me", {
        headers: {
  ...
  const login = async (username: string, password: string): Promise<boolean> => {
    setIsLoading(true)
    try {
      const response = await fetch("http://14.187.198.210:12122/api/auth/login-json", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        // Lấy thông tin user
        const userResponse = await fetch("http://14.187.198.210:12122/api/users/me", {
          headers: {

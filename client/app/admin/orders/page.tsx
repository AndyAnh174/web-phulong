...
export default function AdminOrdersPage() {
  ...
  const fetchOrders = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      const response = await fetch(`http://14.187.198.210:12122/api/orders?${params}`, {
        headers: {
  ...
  const fetchServices = useCallback(async () => {
    ...
    try {
      const response = await fetch('http://14.187.198.210:12122/api/services?limit=100', {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      const response = await fetch(`http://14.187.198.210:12122/api/orders/${selectedOrder.id}`, {
        method: "PUT",
        headers: {
  ...
  const handleExportCSV = async () => {
    try {
      ...
      const response = await fetch(`http://14.187.198.210:12122/api/orders/export/csv?${params}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })

...
export default function AdminContactsPage() {
  ...
  const fetchContacts = async () => {
    try {
      ...
      const response = await fetch(`http://14.187.218.183:12122/api/contact/list?skip=${skip}&limit=${itemsPerPage}`, {
        headers: {
  ...
  const handleDeleteContact = async () => {
    ...
    try {
      const response = await fetch(`http://14.187.218.183:12122/api/contact/${selectedContact.id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },

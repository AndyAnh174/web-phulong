...
export default function AdminServicesPage() {
  ...
  const fetchServices = useCallback(async (refresh = false) => {
    try {
      ...
      let url = `http://14.187.198.210:12122/api/services?skip=${skip}&limit=${itemsPerPage}`
  ...
  const fetchPosts = async () => {
    try {
      ...
      const API_BASE = 'http://14.187.198.210:12122'
  ...
  const handleCreateService = async () => {
    try {
      ...
      const response = await fetch("http://14.187.198.210:12122/api/services/", {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const handleUpdateService = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://14.187.198.210:12122/api/services/${selectedService.id}`, {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const handleDeleteService = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://14.187.198.210:12122/api/services/${slug}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const handleToggleActive = async (service: Service) => {
    ...
    try {
      ...
      const response = await fetch(`http://14.187.198.210:12122/api/services/${service.id}`, {
        method: "PUT",
        headers: {
  ...
  const handleToggleFeatured = async (service: Service) => {
    ...
    try {
      ...
      let response = await fetch(`http://14.187.198.210:12122/api/services/${service.id}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      if (!response.ok) {
        ...
        response = await fetch(`http://14.187.198.210:12122/api/services/${service.id}`, {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const handleSubmitPrinting = async () => {
    ...
    try {
      ...
      const url = printingForm.id ? `http://14.187.198.210:12122/api/printing/${createSlug(printingForm.title)}` : 'http://14.187.198.210:12122/api/printing';
  ...
  const handleDeletePrinting = async (post: PrintingPost) => {
    ...
    try {
      ...
      const res = await fetch(`http://14.187.198.210:12122/api/printing/${slug}`, {
  ...
  const handleToggleVisible = async (post: PrintingPost) => {
    try {
      const slug = createSlug(post.title);
      const res = await fetch(`http://14.187.198.210:12122/api/printing/${slug}/visibility`, {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              ...
              <div className="space-y-2">
                ...
                {selectedService?.image && !removeCurrentImage && !selectedImage && (
                  <div className="relative">
                    <img 
                      src={selectedService.image.url.startsWith('http') ? selectedService.image.url : `http://14.187.198.210:12122${selectedService.image.url}`}
                      ...
                      <div className="flex gap-2 mt-2 flex-wrap">
                        {existingImages.map((img) => (
                          <div key={img.id} className="relative group">
                            <img 
                              src={img.image?.url?.startsWith('http') ? img.image.url : `http://14.187.198.210:12122${img.image?.url}`}
                    ...
                    <TableRow key={post.id}>
                      ...
                      <TableCell>{post.creator?.username || ''}</TableCell>
                        ...
                        {post.images && post.images.length > 0 ? (
                          <img 
                            src={post.images[0].image.url.startsWith('http') ? post.images[0].image.url : `http://14.187.198.210:12122${post.images[0].image.url}`}

...
export default function FeaturedServices() {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        const response = await fetch("http://14.187.207.48:12122/api/services?featured=true&limit=6")
            ...
            >
              <Card className="flex flex-col h-full overflow-hidden border-0 shadow-xl hover:shadow-2xl transition-all duration-500 bg-white/90 backdrop-blur-sm">
                ...
                <div className="relative h-56 bg-gray-100 flex items-center justify-center overflow-hidden">
                  {service.image?.url ? (
                    <Image
                      src={service.image.url.startsWith('http') ? service.image.url : `http://14.187.207.48:12122${service.image.url}`}
                      alt={service.name}
                      fill
